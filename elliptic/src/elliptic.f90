program elliptic
    ...
    select case (problem)
        ...
        case (2)
            call poisson_five_pt(Nx, Ny, hx, hy, zero, prob2_f2, prob2_u,   &
                A, B)
    ...
    print "(3i8,f20.15,f15.3)", Nx + 1, Ny + 1, iterations, norm(X - soln), &
        elapsed_time
...
contains
    ...
    real(kind=DP) function prob2_f2(x, y)
        real(kind=DP), intent(in) :: x, y
        prob2_f2 = 2 * pi**2
    end function prob2_f2
...
#ifdef DEBUG
                    if (verbose == 3) then
                        print fmt, -iteration, norm(estimate - soln),       &
                            estimate - soln
                    end if
#endif
    ...
    ! Generate 5-pt Poisson coefficient matrix for the PDE
    ! -(u_xx + u_yy) = f1(x,y) + f2(x, y)u with Dirichlet BC g(x,y)
    ...
    subroutine process_options
        ...
        do while (i <= command_argument_count())
            ...
            select case (arg)
                ...
                case ("--version")
                    print *, 'elliptic (git ', git_revision, ')'
    ...
    subroutine usage
        print *, "Usage: elliptic [-i] [-j | -g | -c] [-v] [-n mesh_size]"
